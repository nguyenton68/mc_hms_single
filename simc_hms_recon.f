        subroutine simc_hms_recon (delta_p,delta_t,delta_phi,y_tgt
...
C Argument definitions.
        ...
        real*8        delta_p,delta_t,delta_phi,y_tgt ! delta, dy/dz,dx/dz,y
        real*8        fry                        !vertical position at target (+y=down)
        real*8  xs,dxdzs,ys,dydzs  ! hut coordinates (units cm,rad,cm,rad)
C Cosy reconstruction matrix elements.

        integer*4 max_elements
        parameter (max_elements = 1000)

        real*8                coeff(4,4,max_elements)
        integer*2        expon(4,5,max_elements)
        integer*4        n_terms(4),max_order
...
C First time through, read in coefficients from data file.
        ...
        if (firsttime) then
           write(*,*) ' Loading optics recon matrix recon_cosy.dat '
          if (.not.locforunt(chan)) stop 'MC_HMS_RECON: No I/O channels!'
          open (unit=chan,file='recon_cosy.dat')
...
C Make sure hut(5) is non-zero, to avoid taking 0.0**0 (which crashes)
        ...
        hut(1) = xs/100.d00                !cm --> m
        hut(2) = dxdzs                        !slope ("radians")
        hut(3) = ys/100.d00                !cm --> m
        hut(4) = dydzs                        !slope ("radians")

        hut(5) = fry/100.d00        !vert. position at target(cm-->m) pointing down
...
C Load output values.
        ...
        y_tgt          = sum(2)*100.d00                !m --> cm
        delta_t   = sum(3)                !slope ("radians")
        delta_p   = sum(4)*100.d00                !percent deviation
      return
