...
!dec	if (decay_flag .and. .not.dflag) then	!check for decay
...
!dec	  if (zd.le.0) write(6,*) 'transp distance<0:  automatic decay! BAD!'
!dec	  z_decay = -1.*dlen*log(1-grnd())
!dec
!dec	  if (z_decay .le. zd/2) then	!decay in first half
!dec	    dflag=.true.
...
!dec	if (decay_flag .and. .not.dflag) then	!check for decay
!dec	  if (z_decay .gt. zd+delta_z) then	!don't decay within magnet.
!dec	    decdist(30)=decdist(30)+zd
!dec	  else					!apply decay at end/magnet.
!dec	    dflag=.true.
!dec	    decdist(30)=decdist(30)+z_decay
!dec
!dec	    rph = grnd()*2.*pi
