cc	subroutine mc_hms_recon (delta_p,delta_t,delta_phi,y_tgt,fry,frx,p_spec,m2)
...
c target field correction sub. variables
        integer*4 bdl,spect
        real*8 xtgt
	real*8 frx,p_spec,m2
        real*8 x_fp,dx_fp,y_fp,dy_fp
...
C First time through, read in coefficients from data file.
        ...
        if(firsttime)write(6,*) firsttime
        if(first_time_hms)write(6,*) first_time_hms   
        
c        firsttime = .true.

	if (firsttime) then
          write(6,*) "Reading Reconstruction Matrix"
	  if (.not.locforunt(chan)) stop 'MC_HMS_RECON: No I/O channels!'
	  open (unit=chan,status='old',file='recon_cosy.dat')
...
! Read in coefficients and exponents.
     ...
     >			expon(specnum,5,n_terms(specnum)))
	  enddo
cc	  write(6,*) 'HMS: N_TERMS, MAX_ORDER = ',n_terms(specnum),max_order
	  close (unit=chan)
	  firsttime = .false.
	endif
...
C Make sure hut(5) is non-zero, to avoid taking 0.0**0 (which crashes)
	...
	hut(3) = ys/100.		!Meters
	hut(4) = dydzs			!slope
c	hut(5) = fry/100.		!vert. position at target
	hut(5) = yrecs/100.		!vert. position at target
...
c	call track_to_tgt(sum(4)*100,sum(2)*100,sum(3),sum(1),frx,fry,p_spec,m2,
c     >   cos_ts,sin_ts,spect,ok,x_fp,dx_fp,y_fp,dy_fp,xtgt,bdl)
...
c	call track_to_tgt(delta_p,y_tgt,delta_t,delta_phi,frx,fry,p_spec,m2,
c     >   cos_ts,sin_ts,spect,ok,x_fp,dx_fp,y_fp,dy_fp,xtgt,bdl)
